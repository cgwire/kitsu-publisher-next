...
const mutations = {
  ...
  [SET_SEQUENCE_SEARCH](state, { sequenceSearch, production }) {
    ...
    const result = indexSearch(state.sequenceIndex, keywords) || state.sequences
    ...
    state.searchSequenceFilters = getFilters({
      ...
      taskTypes: [],
      taskStatuses: [],
      descriptors: production.descriptors.filter(
  ...
  [SET_EPISODE_SEARCH](state, episodeSearch) {
    ...
    const result = indexSearch(state.episodeIndex, keywords) || state.episodes
