    ...
    this.readClient = function() {
        ...
        if (this.socket.canReadLine()) {
            ...
            if (this.routes.hasOwnProperty(request.url.path())) {
                if (this.routes[request.url.path()]["methods"].indexOf(request.method) >= 0 ) {
                    try {
                        result = this.routes[request.url.path()]["function"](request.method, request.url)
                        ...
                        if (e instanceof MissingQueryError) {
                            ...
                            this.socket.write(new QByteArray("HTTP/1.1 500 Internal Server Error\r\n"))
                        }
                    }
                    this.socket.write(new QByteArray("Content-Type: application/json; charset=\"utf-8\"\r\n"))
                    ...
                    this.socket.write(new QByteArray("Content-Type: text/html; charset=\"utf-8\"\r\n"))
                    this.socket.write(new QByteArray("\r\n"))
