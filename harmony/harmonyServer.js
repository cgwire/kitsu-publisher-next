...
server.add_route("/get_extensions", ["GET"], function(method, url) {
        return (["true", "1", "yes"].indexOf(url.queryItemValue("is_video")) >= 0)?[".mov"]:[".png"]
    }
)
...
server.add_route("/take_render_screenshot", ["GET"], function(method, url) {
        ...
        if (!renderer) {
            throw new HTTPExceptions.MissingQueryError("Missing query argument <renderer>")
        }
        extension = url.queryItemValue("extension")
        if (!extension) {
            throw new HTTPExceptions.MissingQueryError("Missing query argument <extension>")
        }
        ...
        use_colorspace = ["true", "1", "yes"].indexOf(url.queryItemValue("use_colorspace")) >= 0
        return {"file": $.scene.exportLayoutImage(output_path, undefined, $.scene.currentFrame, use_colorspace, 1)._path}
    }
)
...
server.add_route("/take_render_animation", ["GET"], function(method, url) {
       ...
       if (nodes_str.indexOf(renderer)===-1) {
            throw new HTTPExceptions.MissingQueryError("Can't create QuickTime export ("+output_path+") because the renderer ("+renderer+") doesn't exist")
        }
        extension = url.queryItemValue("extension")
        if (!extension) {
            throw new HTTPExceptions.MissingQueryError("Missing query argument <extension>")
